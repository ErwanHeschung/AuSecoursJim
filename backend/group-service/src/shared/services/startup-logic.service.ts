...
import { Table } from 'src/groups/schemas/table.schema';
...
export class StartupLogicService implements OnApplicationBootstrap {
  ...
  createGroup(
    ...
    group.tables = this.generateTables(numberOfPersons);
    return group;
  }

  private generateTables(
    numberOfPersons: number,
    capacityPerTable: number = 5,
  ) {
    const tables = [];
    const tableCount = Math.ceil(numberOfPersons / capacityPerTable);
